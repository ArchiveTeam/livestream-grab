...
LUA_SHA1 = get_hash(os.path.join(CWD, 'livestream.lua'))
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'account':
                wget_args.extend(['--warc-header', 'livestream-account: '+item_value])
                wget_args.append('https://livestream.com/api/accounts/'+item_value)
            elif item_type == 'event':
                account, event = item_value.split(':')
                wget_args.extend(['--warc-header', 'livestream-event: '+event])
                wget_args.append('https://livestream.com/api/accounts/{}/events/{}'.format(account, event))
            elif item_type == 'video':
                account, event, video = item_value.split(':')
                wget_args.extend(['--warc-header', 'livestream-video: '+video])
                wget_args.append('https://livestream.com/api/accounts/{}/events/{}/videos/{}'.format(account, event, video))
            elif item_type == 'status':
                account, event, status = item_value.split(':')
                wget_args.extend(['--warc-header', 'livestream-status: '+status])
                wget_args.append('https://livestream.com/api/accounts/{}/events/{}/statuses/{}'.format(account, event, status))
            elif item_type == 'image':
                account, event, image = item_value.split(':')
                wget_args.extend(['--warc-header', 'livestream-image: '+image])
                wget_args.append('https://livestream.com/api/accounts/{}/events/{}/images/{}'.format(account, event, image))
            elif item_type == 'asset':
                url = 'https://' + item_value
                wget_args.extend(['--warc-header', 'livestream-asset: '+url])
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/1/1f/Livestream-icon-red-background.png" height="50px" title=""/>
        <h2>Livestream <span class="links"><a href="https://livestream.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/livestream/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Livestream">Wiki</a></span></h2>
        <p>Archiving Livestream.</p>
    '''
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://{}/{}/multi={}/'
        .format(TRACKER_HOST, 'arkivertest5', MULTI_ITEM_SIZE),
        downloader, VERSION),
